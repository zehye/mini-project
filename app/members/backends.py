...
class FacebookBackend:
    def authenticate(self, request, code):
        ...
        def get_access_token(code):
            ...
            params = {
                'client_id': settings.secrets['FACEBOOK_APP_ID'],
                'redirect_uri': 'http://localhost:8000/members/facebook-login/',
                'client_secret': settings.secrets['FACEBOOK_APP_SECRET_CODE'],
                'code': code,
            }
            print('!!!!!!code:', code)
        ...
        def debug_token(token):
            ...
            params = {
                'input_token': token,
                'access_token': '{}|{}'.format(
                    settings.secrets['FACEBOOK_APP_ID'],
                    settings.secrets['FACEBOOK_APP_SECRET_CODE'],
                )
            }
            response = requests.get(url, params)
